...
Для Full, multi-slice или изменённой architecture/ownership/compatibility boundary перед первым production write установи `FREEZE_STATUS: approved` только после независимого Architecture Guardian approval exact `plan_revision`. Для bounded Light/Standard без таких изменений используй `FREEZE_STATUS: n/a`; поддержанные RCA и acceptance остаются обязательными. Missing либо unresolved cross-slice поле оставляет Full freeze `pending`; Test-maker и Implementor не запускаются.
...
Implementor пишет production code и минимальные regression tests вместе с fix. Orchestrator выполняет inline `RiskMatrix`; отдельные Architect, Task Assessor, Guardian и Test-maker назначаются только когда без независимого решения нельзя безопасно продолжать. Test-maker нужен лишь для protected critical baseline. Orchestrator не пишет production code или tests.
