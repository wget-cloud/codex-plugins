...
Минимум для production change в изменённом module: `go test -race ./...`, `go vet ./...`, `golangci-lint run`, `go build ./cmd/...` и coverage command/threshold из актуальных workflows и `services.json.coverageMin`. Для migrating service применяй зафиксированный ratchet, не универсальные 90%. Выполняй команды из корня module и проверяй только затронутые runtime concerns.
