...
Оркестратор владеет WorkItem, DecisionSnapshot, ResumeCapsule и EfficiencyBudget, но не пишет production code/tests. Стандартный workflow — один Implementor на Terra, который делает связный tranche и 2–5 минимальных tests. Orchestrator выполняет inline RiskMatrix и targeted verification; один Reviewer либо specialist добавляется только для нетривиального риска. Максимум три assignments на tranche, 10 coordination decisions и один correction batch существующему Implementor; полный pipeline после finding не перезапускается. Sol требует подтверждённого blocker escalation. Каждый агент возвращает только назначенный artifact/verdict с current assessment revision.
