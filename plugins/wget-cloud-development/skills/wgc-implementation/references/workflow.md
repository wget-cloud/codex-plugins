...
- business rule помещается в transport или adapter;
- межсервисный workflow помещается в доменный сервис вместо orchestrator;
- один `services/*` импортирует другой service module напрямую;
- внешний SDK/generated client определяет application port вместо consumer-owned interface;
- stateless adapter получает искусственный domain layer или orchestration service присваивает правила доменных владельцев;
- service-specific behavior переносится в `platform` без второго реального consumer;
...
1. Backward-compatible source-of-truth Protobuf/HTTP/event contract.
2. Reproducible generated artifacts и provider implementation.
3. `platform` primitive только вместе с реальным consumer и consumer evidence.
4. Независимые service modules отдельными DAG slices без direct cross-service imports.
5. Per-service candidate image, SBOM/provenance/security evidence.
6. GitOps desired state с immutable image identity после разрешения.

Это не универсальный порядок: architect обязан обосновать DAG. Один Git repository не отменяет независимость module/image/deployment boundaries; contracts, platform и repository tooling расширяют affected-service matrix только на доказанных consumers.
