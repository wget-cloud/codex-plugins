...
Маршрутизируй все роли по минимальной достаточной GPT-5.6 lane. Стандартная разработка, включая Full по размеру, остаётся на Terra; размер задачи сам по себе не разрешает Sol. Orchestrator использует `balanced`. Активная задача не переключает модель задним числом.
