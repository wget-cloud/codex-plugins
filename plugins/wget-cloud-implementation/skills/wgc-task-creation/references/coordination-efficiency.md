...
До execution задай `MAX_AGENT_ASSIGNMENTS`, `MAX_WAIT_CALLS`, `MAX_EXPENSIVE_CHECKS`, `MAX_REWORK_ROUNDS` и `CHECKPOINT_BOUNDARY`. Default обычного implementation/item slice: 4 assignments, 6 wait calls, 1 дорогая final suite, 1 correction/recheck. Full RCA, product discovery или новый specialist risk получает отдельный обоснованный budget extension.
...
Используй cursor-based event-driven wait: 45–60 секунд первый интерактивный wait, затем exponential backoff. После двух unchanged waits не опрашивай assignment до события или checkpoint boundary. Не делай повторный `list_agents` без нового сигнала и не отправляй status-only follow-up.
...
Перед параллельными read-only gates один раз зафиксируй `DIFF_IDENTITY`. Guardian diff и specialist/QA назначаются только для затронутого concern; неизменившийся approval не повторяется. Для stable finding допустимы initial verdict, одна correction и один recheck; следующее повторение требует contradiction report и rescope/user decision.
...
На границе большого сервиса, epic batch или длительного этапа выпусти компактный `HandoffCapsule`: outcome, tree/item/tracker revisions, remaining scope/risks, valid evidence и next input. Следующий этап не получает историю внутренних correction loops.
