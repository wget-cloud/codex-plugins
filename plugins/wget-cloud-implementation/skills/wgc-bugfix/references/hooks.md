...
Reviewed recovery marker для `twc-wise-finch` также не является общим bugfix/incident bypass. Он привязан только к annotated tag `twc-wise-finch-argocd-recovery-2026-08-17.1` (tag object `344a7e5f87e6c9212dd1ac22256336faad0eb002`, peeled commit `925f7a2949c6ff50b76e55ccec80abdfff59178b`) и семи приложениям до `argocd-public`. В Argo core-mode stage 1 одна переводит root на tag; все stages делают exact sync без revision/prune и wait с timeout 600, pre/post tag attestation, full Application/source/state gates. Bundles `controllers`, `vault-restore`, `eso-ready`, `data`, `apps`, `full` исключены.
