...
Отдельный marker `WGC_GITOPS_STAGED_SYNC_APPROVED=1` также не является incident-repair bypass. Он относится только к заранее проверенной storage-only последовательности `twc-wise-finch` на revision `6c2c3e9dadde2eec3d13fde830bc6db0392b13b8` и принимается лишь внутри exact `/usr/bin/env -i` команды root-owned runner после fd-attestation `/bin/ls`, canonical non-writable/non-deletable `/` → `/usr` → `/usr/bin` chain и самих `/usr/bin/env`/`python3`. Runner через fd/hash/identity и exact Darwin ACL gates сверяет immutable install manifest, versioned CLI/kubeconfig, полный normalized live Argo Application contract и exact app-of-apps tracking owner, разрешает ровно следующий Application и заканчивается на storage smoke. Raw `argocd`, ingress и остальные приложения всегда остаются заблокированы этим исключением.

Reviewed recovery marker для `twc-wise-finch` также не является общим bugfix/incident bypass. Он привязан только к annotated tag `twc-wise-finch-argocd-recovery-2026-08-20.2` (tag object `37c2ee42cb542d30ca200c06e1430d151428a70c`, peeled commit `6247abd4aec30e6a75aeba70123676019762f1a6`) и семи приложениям до `argocd-public`. В Argo core-mode stage 1 одна переводит root на tag; все stages делают exact sync без revision/prune и wait с timeout 600, pre/post in-process GitHub API attestation exact tag-ref/tag-object/peeled-commit lineage и full Application/source/state gates. API redirects запрещены, JSON media type и 64 KiB body bound обязательны, а documented metadata не может конфликтовать с exact identity URL/type/SHA. Token читается один раз из root:wheel `0600` stable `O_NOFOLLOW` FD с exact pre/post identity/size/length и ACL user `FFFFEEEE-DDDD-CCCC-BBBB-AAAA000001F5`, существует только в Authorization header и не попадает в failure/output/log. Bundles `controllers`, `vault-restore`, `eso-ready`, `data`, `apps`, `full` исключены.

Timeweb source manifest остаётся неисполняемым `approvalTemplate`; hook блокирует его digest и принимает только отдельно материализованный fresh ≤300s approval с published foundation shapes и exact generated `TimewebRouterRecoveryPlan`. Production main читает materialized manifest одним stable FD и выполняет только ordered plan. Router ID открывается динамически, token читается стабильным FD из root:wheel `0600` файла с exact read-only ACL user `FFFFEEEE-DDDD-CCCC-BBBB-AAAA000001F5` и запрещён в Git/argv/env/log. Final GET формирует exact `TimewebRouterRecoveryPoststate`; stage 7 recomputes `routerPlan`/`routerPoststate`/promotion digest lineage. Изменение/reinstall plugin в текущей задаче не обновляет уже загруженный hook: smoke выполняется новой задачей; никаких cluster/router mutations это не разрешает.
