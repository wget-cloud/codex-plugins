...
Узкий reviewed ingress/router recovery — отдельный контракт, не расширение обычного bugfix deployment. Он разрешает только семь Argo core-mode стадий `twc-wise-finch` до `argocd-public` по annotated tag `twc-wise-finch-argocd-recovery-2026-08-20.2` / tag object `37c2ee42cb542d30ca200c06e1430d151428a70c` / peeled commit `6247abd4aec30e6a75aeba70123676019762f1a6`, с единственным stage-1 `app set`, sync без revision/prune, wait, pre/post in-process GitHub API attestation без redirects и свежими full-state attestations. HTTP adapter ограничен approved JSON media types, 64 KiB и deterministic timeout; safe metadata не расширяет exact identity URL/type/SHA. Token читается один раз из root:wheel `0600` stable `O_NOFOLLOW` FD с exact pre/post identity/size/length и ACL и используется только в Authorization header; ошибки остаются privacy-safe. Поздние bundles `controllers`, `vault-restore`, `eso-ready`, `data`, `apps`, `full` не разрешены.
