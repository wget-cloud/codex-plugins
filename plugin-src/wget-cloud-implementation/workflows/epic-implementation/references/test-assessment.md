...
Для каждого frozen epic item Full или add/update отдельный Test-maker выпускает per-item `TestAssessment` и выбирает `add | update | reuse | none`. Для Light/Standard none/reuse его выпускает Task Assessor. Aggregate assessment или verdict одного item не переносится на другой.
