...
Оркестратор владеет EpicRun, DecisionSnapshot, ResumeCapsule, EfficiencyBudget и transitions. Один item/slice имеет одного write-owner; Reviewer проверяет frozen candidate, остальные gates запускаются по item risk и invalidated concern. Каждый агент возвращает только назначенный artifact/verdict с current assessment revision. Domain profile не расширяет write permissions. Protected-test author, Reviewer и Guardian независимы; Architect не утверждает свой план. Максимум три активных субагента; [model/context policy](references/model-routing.md).
